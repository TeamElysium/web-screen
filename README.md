...
`web-screen`은 tmux 세션을 웹 브라우저에서 관리하고 접속하기 위한
모바일 친화 웹 터미널입니다.
...
- 브라우저에서 tmux 세션 목록 확인, 생성, 삭제
- xterm.js 기반 터미널 접속
- 브라우저를 닫거나 연결이 끊겨도 tmux 세션 유지
...
4. 세션을 만들거나 기존 tmux 세션에 접속합니다.
5. 세션 안에서 `claude`, `codex` 같은 TUI를 실행합니다.
6. 이동 중에는 브라우저를 다시 열어 진행 상황을 확인하고, 필요할 때 입력합니다.

브라우저 연결은 tmux에 attach하는 역할만 합니다. 브라우저를 닫으면 서버가
tmux에서 detach하고, 세션과 그 안의 프로세스는 계속 실행됩니다.
...
기본 backend는 tmux입니다. 기존 GNU screen 동작이 필요하면
`TERMINAL_BACKEND=screen`을 설정해서 실행할 수 있습니다.
...
웹 UI에서 `+ Session`을 눌러 새 tmux 세션을 만들 수 있습니다. 세션 이름은
영문, 숫자, 하이픈, 밑줄만 허용됩니다.

이미 터미널에서 만든 기본 tmux 세션도 목록에 나타납니다.
...
web-screen 전용 tmux socket으로 격리하고 싶으면 서버 실행 시
`TMUX_SOCKET_NAME`을 설정하고, 터미널에서도 같은 `-L` 값을 사용합니다.

```bash
TMUX_SOCKET_NAME=web-screen ALLOWED_IPS=127.0.0.1 npm run dev
tmux -L web-screen new-session -s claude
```

screen backend를 명시적으로 사용할 경우에는 기존처럼 screen 세션을 사용할 수
있습니다.

```bash
TERMINAL_BACKEND=screen ALLOWED_IPS=127.0.0.1 npm run dev
```
...
- 기본 backend는 tmux입니다.
- GNU screen backend는 `TERMINAL_BACKEND=screen`으로 남아 있지만, Claude Code
  2.1.x 계열에서는 screen 자체의 렌더링 한계로 화면이 깨질 수 있습니다.
...
src/lib/screen-manager.ts    # 세션 관리 API
src/lib/terminal-backend.ts  # tmux/screen backend 명령
